...
import '../../foundation/nebula_responsive.dart';
...
///
/// ```dart
/// final dimension = NebulaDimension(1.125); // desktop scale
/// ```
///
/// When [factor] is `1.0` (default), values match [_Dimension] exactly.
class NebulaDimension {
  /// Creates a scaled dimension set by applying [factor] to every base token.
  ///
  /// Defaults to `1.0` (base / tablet scale) when omitted.
  const NebulaDimension([this.factor = 1.0]);

  /// Resolves the scale factor from [context] via [NebulaResponsive].
  factory NebulaDimension.of(BuildContext context) {
    return NebulaDimension(NebulaResponsive.scaleOf(context));
  }
...
/// Convenience extension to access [NebulaDimension] directly from a
/// [BuildContext].
///
/// ```dart
/// final dimension = context.dimension;
/// padding: EdgeInsets.all(dimension.md),
/// ```
extension NebulaDimensionContext on BuildContext {
  /// Returns a [NebulaDimension] scaled to the current breakpoint.
  NebulaDimension get dimension => NebulaDimension.of(this);
}
...
/// ...
/// ```
extension NebulaDimensionDouble on double {
